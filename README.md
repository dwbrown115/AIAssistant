...
# Optional: from RS recovery phases after initial lock, widen beam recency anchoring to reduce
# beam_not_recent suppression churn while re-stabilizing baseline behavior.
MV_TC0M2_PLUS_RECENT_EXIT_STEPS=96
# Optional: stage-prefix mapping for beam/MV gate progression on current RS recovery ladder.
MV_OBJECTIVE_MV_GATE_STAGE_PREFIX=rs0.
MV_BEAM_RETIREMENT_STAGE_PREFIX=rs7.
...
- For a short post-recovery lock pass focused on warning-free hard-window stability, see `phase_plans/RS_POST_RECOVERY_STABILITY_LOCK_MINI_PHASE_PLAN_V1.md`.
